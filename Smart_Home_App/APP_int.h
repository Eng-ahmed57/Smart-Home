...
/*
   Default Flag = 0
   input (a) -> Motor OFF
   input (b) -> Motor ON with HIGH SPEED 70% Duty cycle
   input (c) -> Motor ON with LOW SPEED 30% Duty Cycle
   input (d) -> Auto Mode based on Temperature sensor reading
 */
 u8 Motor_State_Flag;
...
/*
   Default Flag = 0
   input (e) -> LED OFF
   input (f) -> LED ON - Auto mode
 */
 u8 LED_State_Flag;
 u8 LED_previous_State_Flag;

/* If user enterd (g) Temp flag set to 6
   Default to 0
 */
...
/******************************************
  syntax :			ES_t sytem_init();
  Description :		init all system moduels
