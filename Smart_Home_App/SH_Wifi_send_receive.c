...
/* Read user input via mobile phone */
void Read_Wifi(void)
{
	...
	/* Check Inputs for Menu_Request_Flag */
	if (Wifi_Wait_Data(Menu_Requested))			Request_Menu = Menu_Requested;
	
	Wifi_Buffer_Reset();
}


/* Send temperature to user if Temperature requested flag is set */
...
void Show_Menu_Wifi()
{
	if (Request_Menu == Menu_Requested)
	{
		Wifi_Send_Data2("\nWelcom Ahmed:\n a: Motor OFF \n b: Motor High speed \n c: Motor Low Speed \n d: Motor Auto \n e: LED OFF \n f: LED Auto \n g: Show Temperature\n m: Show Menu\n z: Logout\n");
		Request_Menu =0;
	}
